RSpec.describe Foobara::Callback::Registry::SingleAction do
  ...
  describe "#register_callback" do
    ...
    context "when given good action" do
      it "registers it and can return it even when unioned with other actions" do
        expect {
          register_it
        }.to change { registry.specific_callback_set_for.size }.from(0).to(1)

        set = registry.unioned_callback_set_for
