          ...
          def process_value(...)
            super.tap do |outcome|
              if outcome.success?
                ...
                namespace = domain || Foobara

                type_namespace = domain&.foobara_type_namespace || TypeDeclarations::Namespace.global
                model_symbol = model_class.model_symbol

                if existing_model_type
                  if existing_model_type.declaration_data != type.declaration_data && domain.foobara_type_registered?(existing_model_type)
                    registry = domain.foobara_type_namespace.registry_for_symbol(model_symbol)
                    registry.unregister(model_symbol)

                    foobara_type_namespace.register_type(model_symbol, type)
                    namespace.foobara_register(type) # TODO: will this register it twice??
                    type.foobara_parent_namespace = namespace
                  end
                  ...
                  type_namespace.register_type(model_symbol, type)
                  namespace.foobara_register(type) # TODO: will this register it twice??
                  type.foobara_parent_namespace = namespace
                end
              end
